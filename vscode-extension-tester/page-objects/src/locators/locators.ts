...
export function hasAttribute(attr: string, value?: string, locator?: By): (el: WebElement) => Promise<boolean> {
    return async (el: WebElement) => {
        ...
        const attrValue: string = await el.getAttribute(attr);
...
export function hasClass(
    classOrPredicate: string | ((klass: string) => boolean),
    locator?: By,
): (el: WebElement) => Promise<boolean> {
    return async (el: WebElement) => {
        el = locator ? el.findElement(locator) : el;
        const klasses: string = await el.getAttribute("class");
        const segments: string[] = klasses?.split(/\s+/g);

        const predicate: (val: string) => boolean =
            typeof classOrPredicate === "string"
                ? (klass: string): boolean => klass === classOrPredicate
                : classOrPredicate;
...
export function hasElement(
    locatorSelector: (l: Locators) => By,
): (el: WebElement, locators: Locators) => Promise<boolean> {
    return async (el: WebElement, locators: Locators) => (await el.findElements(locatorSelector(locators))).length > 0;
}
