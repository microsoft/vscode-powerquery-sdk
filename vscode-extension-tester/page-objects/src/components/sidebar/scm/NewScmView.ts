...
export class NewScmView extends ScmView {
    ...
    override async getProviders(): Promise<ScmProvider[]> {
...
export class SingleScmProvider extends ScmProvider {
     ...
     */
    override async getType(): Promise<string> {
        return '';
    }

    override async takeAction(title: string): Promise<boolean> {
    ...
    override async openMoreActions(): Promise<ContextMenu> {
        const view = this.enclosingItem as NewScmView;
        return await new MoreAction(view).openContextMenu();
    }

    override async getChanges(staged: boolean = false): Promise<ScmChange[]> {
...
export class MultiScmProvider extends ScmProvider {
    ...
    override async commitChanges(message: string): Promise<void> {
    ...
    override async getChangeCount(staged: boolean = false): Promise<number> {
        const locator = staged ? ScmProvider.locators.ScmView.stagedChanges : ScmProvider.locators.ScmView.changes;
