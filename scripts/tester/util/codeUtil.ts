...
import { VSRunner } from "../suite/VSRunner";
...
export class CodeUtil {
    ...
    constructor(
        ...
        type: ReleaseQuality = ReleaseQuality.Stable,
        ...
        if (type === ReleaseQuality.Stable) {
            this.codeFolder = path.join(
                this.downloadFolder,
                process.platform === "darwin" ? "Visual Studio Code.app" : `VSCode-${this.downloadPlatform}`,
            );
        } else {
            this.codeFolder = path.join(
                this.downloadFolder,
                process.platform === "darwin"
                    ...
                    : `VSCode-${this.downloadPlatform}-insider`,
            );
        }
    }
    ...
    open(...paths: string[]): void {
        ...
        let command = `${this.getCliInitCommand()} -r ${segments} --user-data-dir="${path.join(
            this.downloadFolder,
            "settings",
        )}"`;
    ...
    async runTests(testFilesPattern: string[], runOptions: RunOptions = DEFAULT_RUN_OPTIONS): Promise<number> {
        ...
        const literalVersion =
            runOptions.vscodeVersion === undefined || runOptions.vscodeVersion === "latest"
     ...
     */
    async downloadAndUnzipVSCode(version: string = "latest"): Promise<string> {
        this.executablePath = await downloadAndUnzipVSCode(version);
        this.cliPaths = resolveCliArgsFromVSCodeExecutablePath(this.executablePath);
    ...
    private getExistingCodeVersion(): string {
        let command = this.cliPaths.join(" ");
        let out: Buffer;
        try {
